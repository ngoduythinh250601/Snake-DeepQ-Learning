...
class DQNSolver(BaseSolver):
    def __init__(self, snake):
        ...
        if self.snake.map.num_rows - 2 == 8:
            self.PATH_VAR = os.path.join(_DIR_LOG, "solver-var-mapmode-%d.json")
            self.PATH_NET = os.path.join(_DIR_LOG, "solver-net-mapmode-%d")
        elif self.snake.map.num_rows - 2 == 12:
            self.PATH_VAR = os.path.join(
                _DIR_LOG, "solver-var-mapsize-12x12-mapmode-%d.json"
            )
            self.PATH_NET = os.path.join(
                _DIR_LOG, "solver-net-mapsize-12x12-mapmode-%d"
            )
        ...
        # Maximum learning steps (require >= self._RESTORE_STEP)
        self._MAX_LEARN_STEP = 3000000
        # Which learning step to restore (0 means not restore)
        self._RESTORE_STEP = 3000000
        ...
        self._SHAPE_VISUAL_STATE = (
            self.map.num_rows - 2,
            self.map.num_cols - 2,
            5 if self.snake.map.map_mode > 0 else 4,
        )
        ...
        if self._RESTORE_STEP > 0 and self.snake.map.map_mode != 1:
            self._load_model()
    ...
    def _load_model(self):
        self._net_saver.restore(self._sess, self.PATH_NET % self.snake.map.map_mode)
        with open(self.PATH_VAR % self.snake.map.map_mode, "r") as f:
            var = json.load(f)
        self._epsilon = var["epsilon"]
        self._beta = var["beta"]
        self._learn_step = self._RESTORE_STEP + 1
        log(
            "model loaded | RESTORE_STEP: %d | epsilon: %.6f | beta: %.6f"
    ...
    def _build_net(self, features, output_name, w_init_, b_init_):
        ...
        conv4_flat = tf.reshape(
            ...
            shape=[
                -1,
                (self.snake.map.num_rows - 2 - 6)
                * (self.snake.map.num_cols - 2 - 6)
                * 256,
            ],
            name="conv4_flat",
        )
        ...
        for i in range(1, self.map.num_rows - 1):
            for j in range(1, self.map.num_cols - 1):
                ...
                if t == PointType.EMPTY:
                    ...
                    t == PointType.BODY_LU
                    or t == PointType.BODY_UR
                    or t == PointType.BODY_RD
                    or t == PointType.BODY_DL
                    or t == PointType.BODY_HOR
                    or t == PointType.BODY_VER
                ):
                    visual_state[i - 1][j - 1][3] = 1
                elif t == PointType.WALL:
                    visual_state[i - 1][j - 1][4] = 1
                else:
